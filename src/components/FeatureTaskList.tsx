...
import { Pencil, Trash2, Check, X, Ban } from 'lucide-react'
...
function dueDateBadge(due: string | null, status: string): React.ReactNode {
  ...
  if (!isDone && d.getTime() === today.getTime()) {
    return <span className="inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 ml-1">Due today</span>
  }
...
export default function FeatureTaskList({ featureId, deliverableId, deliverableMandays, deliverablePlannedEnd, userRole, developers }: Props) {
  ...
  // Update default due_date when deliverablePlannedEnd changes
  useEffect(() => {
    setNewTask(prev => ({
      ...prev,
      due_date: deliverablePlannedEnd ? deliverablePlannedEnd.slice(0, 10) : '',
  ...
  async function toggleBlock(task: Task) {
    if (task.is_blocked) {
      ...
      const res = await fetch(`/api/tasks/${task.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ is_blocked: false, blocked_reason: null }),
      })
      if (res.ok) setTasks(prev => prev.map(t => t.id === task.id ? { ...t, is_blocked: false, blocked_reason: null } : t))
    } else {
  ...
  async function saveBlock(taskId: number) {
    const res = await fetch(`/api/tasks/${taskId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ is_blocked: true, blocked_reason: blockReason || null }),
    })
    if (res.ok) {
      setTasks(prev => prev.map(t => t.id === taskId ? { ...t, is_blocked: true, blocked_reason: blockReason || null } : t))
            ...
            >
                ...
                {editingTaskId === task.id ? (
                  ...
                  <div>
                    <div className="flex items-center gap-1 flex-wrap">
                      ...
                      {task.is_blocked && (
                        <span className="inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400">
                          🚫 Blocked
                        </span>
                      )}
                      {task.title}
                    ...
                    {blockingTaskId === task.id && (
                      <div className="mt-1 flex items-center gap-1">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={() => saveBlock(task.id)} className="text-xs px-2 py-1 bg-red-500 text-white rounded">Block</button>
                        <button onClick={() => setBlockingTaskId(null)} className="text-xs px-2 py-1 border border-slate-200 dark:border-navy-600 rounded text-slate-500">Cancel</button>
                      </div>
                    )}
                  </div>
                )}
              ...
              <td className="px-3 py-2">
                <div className="flex items-center gap-1">
                  ...
                  {(userRole === 'manager' || task.assigned_to === currentUserId) && blockingTaskId !== task.id && (
                    <button
                      onClick={() => toggleBlock(task)}
                      className={`p-1 rounded text-xs ${task.is_blocked ? 'text-green-500 hover:text-green-700' : 'text-red-400 hover:text-red-600'}`}
                      title={task.is_blocked ? 'Unblock' : 'Block'}
                    >
