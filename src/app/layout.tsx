...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const [nameRow, logoRow] = await Promise.all([
      prisma.appSetting.findUnique({ where: { key: 'brand_name' } }),
      prisma.appSetting.findUnique({ where: { key: 'brand_logo_url' } }),
    ])
    const name = nameRow?.value ?? 'IT Progress Tracker'
    const logo = logoRow?.value
    return {
      title: name,
      description: `${name} — Project progress tracking`,
