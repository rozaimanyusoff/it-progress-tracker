...
export async function POST(req: NextRequest) {
  ...
  if (!ALLOWED.includes(file.type)) return NextResponse.json({ error: 'Only image files allowed (JPEG, PNG, GIF, WebP, SVG)' }, { status: 400 })
  if (file.size > MAX_SIZE_MB * 1024 * 1024) return NextResponse.json({ error: `Max size is ${MAX_SIZE_MB}MB` }, { status: 400 })

  const ext = file.type === 'image/svg+xml' ? 'svg' : file.name.split('.').pop()
  const filename = `logo-${Date.now()}.${ext}`
  const uploadDir = path.join(process.cwd(), 'public', UPLOAD_PUBLIC_URL, 'brand')
